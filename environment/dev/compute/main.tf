module "compute" {
  ...
  instance_availability_domain = local.availability_domain_ids[each.value.instance_availability_domain]

  instance_cmp_id = local.compartment_ids[each.value.instance_cmp]

  instance_assign_public_ip = each.value.instance_assign_public_ip
  instance_subnet_id        = local.subnet_ids[each.value.instance_subnet].id
